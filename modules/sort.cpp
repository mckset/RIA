...
void SortFolders(int l, int sub){
	if (l == -1)
		return;

	if (sub == -1){
		sort(locations[l].folders.begin(), locations[l].folders.end(), SortTable);
	}else{
		sort(locations[l].folders[sub].folders.begin(), locations[l].folders[sub].folders.end(), SortTable);
	}
...
bool SortTable(Table t1, Table t2){
