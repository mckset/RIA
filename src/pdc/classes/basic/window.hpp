enum Window_Errors{
	WINDOW_ERROR_NONE,
	WINDOW_ERROR_CREATION,
	WINDOW_ERROR_GLAD,

};

float defaultScale = 1.0f;

class Window{
	public:
		bool visible = true, scalable = true, focused = false;
		unsigned int width, height; // Window width and height
		unsigned int vA, vB, eB; // Buffers used to draw to the screen
		GLFWwindow* w; // Window pointer
		Vector2 view; // Window view offset
		float scale = 1.0f; // Window scale

		// Render and input function pointers
		void (*Render)() = nullptr;
		void (*Input)() = nullptr;

		void (*Close)() = nullptr; // Function to go to when the window wants to close

		bool IsFocused(){return glfwGetWindowAttrib(w, GLFW_FOCUSED);}
		void Show(){glfwShowWindow(w); visible = true;}
		void Hide(){glfwHideWindow(w); visible = false;}
		void Toggle(){if (visible) Hide(); else Show();}
		void Resize(int Width, int Height){glfwSetWindowSize(w, Width, Height);}
		void Move(int x, int y){glfwSetWindowPos(w, x, y);}

		void Draw(Color clear = Color{0,0,0,1}){
			ShaderID = -1;

			if (!visible){
				if (focused) focused = false;
				return;
			}
			Use();
			// Background clear
			glClearColor(clear.r,clear.g,clear.b,clear.a);

			// Set to draw buffer
			glClear(GL_COLOR_BUFFER_BIT);

			CurrentWindow = this;

			Render();
			if (focused = IsFocused()){
				FocusedWindow = this;
				if (Input) Input();
				if (!mouse.drag && ((mouse.state == LM_DOWN && mouse.prevState == LM_DOWN) || (mouse.state == RM_DOWN && mouse.prevState == RM_DOWN)))
					mouse.drag = true;
				mouse.prevState = mouse.state;
				keyboard.newKey = -1;
				mouse.scrollX = 0;
				mouse.scrollY = 0;
			}

			glfwSwapBuffers(w);
		}

		int Init(unsigned int wi=Width, unsigned int h=Height, bool vSync = true){
			// Check if window was created
			if (w == NULL){
				if (PDC_DEBUG) printf("Failed to create window\n");
				glfwTerminate();
				return WINDOW_ERROR_CREATION;
			}
			glfwMakeContextCurrent(w);
				
			// Initialize GLAD
			if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)){
				if (PDC_DEBUG) printf("Failed to initialize GLAD\n");
				return WINDOW_ERROR_GLAD;
			}
			if (PDC_DEBUG) printf("[Init] Created Window\n");

			// Sets OpenGL viewport
			glViewport(0, 0, wi, h);
			glfwSwapInterval(vSync); // Disable vSync

			glEnable(GL_BLEND);
			glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

			// Set window flags
			glfwSetWindowAttrib(w, GLFW_RESIZABLE, GLFW_TRUE);

			if (PDC_DEBUG) printf("[Init] Set Flags\n");

			// Set event callbacks
			glfwSetErrorCallback(Error);
			glfwSetFramebufferSizeCallback(w, OnResize);

			// Bind keyboard events
			glfwSetKeyCallback(w, SetKeyboardState);

			// Bind mouse events
			glfwSetMouseButtonCallback(w, SetMouseState);
			glfwSetCursorPosCallback(w, SetCursorPosition);

			if (PDC_DEBUG) printf("[Init] Bound Events\n");

			// Generate vertex array
			glGenVertexArrays(1, &vA);

			// Generate vertex buffer
			glGenBuffers(1, &vB);

			// Generate element buffer
			glGenBuffers(1, &eB);

			if (PDC_DEBUG) printf("[Init] Bound buffers\n");

			width = wi;
			height = h;
			glfwSetWindowSize(w, width, height);
			return 0;
		}

		Vector2 GetScreenCorner(){
			return Vector2{-(float)width/2, -(float)height/2} / scale + Vector2{(float)width/2, (float)height/2} + view;
		}

		// Tells GLFW to use the window
		void Use(){
			glfwMakeContextCurrent(w);
			Width = width;
			Height = height;
			fWidth = (float)width;
			fHeight = (float)height;
			View = &view;
			Scale = (scalable ? &scale : &defaultScale);
			glViewport(0,0, Width, Height);
		}
};