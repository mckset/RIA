...
class Image{
	public:
		...
		// Draws to pixel size
		void Draw(Vector2 position, Vector2 size, Color color = White, float angle = 0, bool fixed = false, Vector2 anchor = Vector2{0, 0}, bool flipX = false, bool flipY = false){
			if (!loaded)
				return;
			...
			if (hFlip || flipX){
				position.x += anchor.x*2 - size.x;
			}

			Vector2 tR = Vector2{!(hFlip || flipX) ? position.x + size.x : position.x, !(vFlip || flipY) ? position.y + size.y : position.y};
			Vector2 tL = Vector2{!(hFlip || flipX) ? position.x : position.x + size.x, !(vFlip || flipY) ? position.y + size.y : position.y};
			Vector2 bR = Vector2{!(hFlip || flipX) ? position.x + size.x : position.x, !(vFlip || flipY) ? position.y : position.y + size.y};
			Vector2 bL = Vector2{!(hFlip || flipX) ? position.x : position.x + size.x, !(vFlip || flipY) ? position.y : position.y + size.y};
		...
		bool LoadImage(string p, bool nearest = false){
			return (LoadImage(p.data(), nearest));
		}
		...
		void SetTexture(unsigned char* data, bool nearest = false){
			glGenTextures(1, &ID);
			glBindTexture(GL_TEXTURE_2D, ID);
		...
		// Checks to see if the whole image is transparent
		bool Empty(){
			glBindTexture(GL_TEXTURE_2D, ID);
			unsigned char *pixels = (unsigned char*) malloc(sizeof(unsigned char) * (width * height) * 4);
			glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
