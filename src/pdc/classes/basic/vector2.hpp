...
class Vector2{
	public:
		...
		Vector2 Truncate(){return Vector2{(float)((int)x), (float)((int)y)};}
		...
		bool Within(Vector2 origin, Vector2 size, bool exact = false){
			if (size.x < 0){
				origin.x += size.x;
				size.x = -size.x;
			}
			...
			return (!exact) ? (x > origin.x && x < origin.x + size.x && y > origin.y && y < origin.y + size.y) :
				(x >= origin.x && x <= origin.x + size.x && y >= origin.y && y <= origin.y + size.y);
		}

};

Vector2 Center(Vector2 start, Vector2 end){return start+((end-start)/2);}
